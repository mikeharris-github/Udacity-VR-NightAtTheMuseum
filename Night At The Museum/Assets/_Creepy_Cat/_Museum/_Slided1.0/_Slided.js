// Slided engine 1.1 par Creepy Cat (C)2015
...
// for VR
public  var movingObject:GameObject;
...
function FacingCamera(){
	...
	// For VR (Camera need to be parented to a gameobject)
	if (movingObject){
		// Moving camera
		movingObject.transform.position = Vector3(currentSlideX, currentSlideY, currentSlideZ);

		// Pointing camera to pivot
		movingObject.transform.LookAt(slidePivot.transform); 

	// If not moving object, i move the camera (Slied script need to be placed on the camera off course..)
	}else {
		// Moving camera
